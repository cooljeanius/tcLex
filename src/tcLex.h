...
/*  Tcl_DString chars;*/
  ...
  int index;            /* Current character position within <chars> */
  int nextIndex;
...
typedef struct TcLex_State {
  ...
  int lastMatched;            /* Last matched rule. We need it
                               * for empty strings to be matched
                               * properly */
  char *bFailed;              /* Rule failure indicators. We need
                               * it for incremental processing to
                               * work properly */
